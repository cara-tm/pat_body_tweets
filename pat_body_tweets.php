	...
	if (preg_match('/^p|li|h[1-6]{1}?$/', $tag)) {
		...
		for ($i=0; $i <= count($body_array)-2; ++$i) {
			...
			if($full == true) {
				...
				$words = urlencode(substr(html_entity_decode($text, ENT_NOQUOTES, 'UTF-8'), 0, 254)).'...';
			}
